...
- `src-tauri/src/git/` — pure git logic, no Tauri dependency, fully unit-testable.
  - `mod.rs` — process runner (`run_git`, `run_git_with_ok_codes` for commands like
    `diff` that exit 1 on success) and the serializable `GitError`.
  ...
  - `testutil.rs` — `TestRepo`: temp-dir fixture isolated from the user's global git
